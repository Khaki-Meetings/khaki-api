...
public class GoogleCalendarRepository {
    ...
    public List<Event> getEvents(String email) throws GeneralSecurityException, IOException {
        if(this.client==null){
            // Build a new authorized API client service.
            final NetHttpTransport HTTP_TRANSPORT = GoogleNetHttpTransport.newTrustedTransport();
             this.client = new Calendar.Builder(HTTP_TRANSPORT, JSON_FACTORY, getCredentials(HTTP_TRANSPORT))
                    .setApplicationName(APPLICATION_NAME)
                    .build();
        }

        DateTime now = new DateTime(System.currentTimeMillis());
        Events events = this.client.events().list(email)
                .setTimeMin(now)
