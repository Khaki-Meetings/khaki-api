...
public interface CalendarEventRepositoryInterface extends JpaRepository<CalendarEventDao, UUID> {
    ...
    String internalCalendarEventsForAttendeeFromWhereClause =
            "  from calendar_event_dao `ced`, person_dao pd, email_dao_people edp, " +
                    "       email_dao ed, calendar_event_participant_dao cepd, domain_dao dd " +
                    " where ced.start between :sDate and :eDate " +
                    "       and pd.id = :organizer " +
    ...
    @Query(
            value = "select ced.id as id," +
                    ...
                    "       pd.last_name as organizerLastName, " +
                    "       concat(ed.user, '@', dd.name) as organizerEmail " +
                    internalCalendarEventsForAttendeeFromWhereClause +
                    " having numberInternalAttendees > 0 "
            ,countQuery = "select ced.id as id," +
            "       ( select count(*) " +
            "           from person_dao, " +
    ...
    Page<CalendarEventsWithAttendeesView> getInternalCalendarEventsForAttendee(
            UUID tenantId, Instant sDate, Instant eDate, UUID organizer, Pageable pageable);


    String externalCalendarEventsForAttendeeFromWhereClause =
            "  from calendar_event_dao `ced`, person_dao pd, email_dao_people edp, " +
                    "       email_dao ed, calendar_event_participant_dao cepd, domain_dao dd " +
                    " where ced.start between :sDate and :eDate " +
                    "       and pd.id = :organizer " +
    ...
    @Query(
            value = "select " +
                    ...
                    "       pd.last_name as organizerLastName, " +
                    "       concat(ed.user, '@', dd.name) as organizerEmail " +
                    externalCalendarEventsForAttendeeFromWhereClause +
                    " having numberInternalAttendees > 0 "
    ...
    Page<CalendarEventsWithAttendeesView> getExternalCalendarEventsForAttendee(
            UUID tenantId, Instant sDate, Instant eDate, UUID organizer, Pageable pageable);


    String allCalendarEventsForAttendeeFromWhereClause =
            "  from calendar_event_dao `ced`, person_dao pd, email_dao_people edp, " +
                    "       email_dao ed, calendar_event_participant_dao cepd, domain_dao dd, " +
                    "       domain_dao_organizations ddo2 " +
                    " where ced.start between :sDate and :eDate " +
                    "       and pd.id = :organizer " +
                    "       and pd.id = edp.people_id " +
                    "       and edp.emails_id = ed.id " +
    ...
    @Query(
            value = "select " +
                    ...
                    "       pd.last_name as organizerLastName, " +
                    "       concat(ed.user, '@', dd.name) as organizerEmail " +
                    allCalendarEventsForAttendeeFromWhereClause
    ...
    Page<CalendarEventsWithAttendeesView> getAllCalendarEventsForAttendee(
            UUID tenantId, Instant sDate, Instant eDate, UUID organizer, Pageable pageable);
